...
It's a good idea to make a sanity plot at this point, garbage into a model leads to garbage out...
```{r}
...
LSMs typically use SWdown, so we will need to convert this to PAR and for our big-leaf scaling we also need to transform this to APAR.
...
df_tair <- df %>%
  mutate(day=as.Date(time, format="%Y-%m-%d")) %>%
  group_by(day) %>%                  # group by the day column
  summarise(Tair=mean(Tair))   # calculate the SUM 
                                     # NB unit conversion

df <- data.frame(time, vpd)
colnames(df)<- c("time", "vpd") 

# Let's use the dplyr lib to aggregate to daily data
df_vpd <- df %>%
  mutate(day=as.Date(time, format="%Y-%m-%d")) %>%
  group_by(day) %>%                  # group by the day column
  summarise(vpd=mean(vpd))   # calculate the mean 
                                     # NB unit conversion

df_tmp <- merge(df_par, df_tair, by="day")
colnames(df_tmp)<- c("day", "PAR", "Tair") 
df <- merge(df_tmp, df_vpd, by="day")
colnames(df)<- c("day", "PAR", "Tair", "vpd") 
...
By comparing the simple LUE model with our more complicated model, can we spot any weaknesses in the model assumptions? For example, why does the LUE have a much stronger seasonality? What assumption do you think drives this? Can you test this by adjusting the LUE equation?
