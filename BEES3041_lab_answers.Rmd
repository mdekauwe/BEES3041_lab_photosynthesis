...
```{r}
conv_hlf_hr_to_daily <- function(df, var, var_name, stat) {
  df_day <- df %>%
    mutate(day=as.Date(time, format="%Y-%m-%d")) %>%
    group_by(day) %>%               # group by the day column
    summarise(var_name=stat(var))   # calculate the sum/mean/max/min 
...
Answer:
```{r}
name <- "data/FR-Pue_2002-2003_FLUXNET2015_Flux.nc"
f <- nc_open(fname)
